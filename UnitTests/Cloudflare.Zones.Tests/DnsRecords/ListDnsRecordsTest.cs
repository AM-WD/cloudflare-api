...
namespace Cloudflare.Zones.Tests.DnsRecords
{
	...
	public class ListDnsRecordsTest
	{
		...
		private CloudflareResponse<IReadOnlyList<DnsRecord>> _response;
		...
		public void Initialize()
		{
			...
			_response = new CloudflareResponse<IReadOnlyList<DnsRecord>>
			{
				Success = true,
				Messages = [
					new ResponseInfo
				...
				Result = [
					new DnsRecord
					{
						...
						ZoneId = "023e105f4ecef8ad9ca31a8372d0c354",
					...
					new DnsRecord
					{
						...
						ZoneId = "023e105f4ecef8ad9ca31a8372d0c356",
						ZoneName = "example.com",
						Name = "example.com",
						Type = DnsRecordType.A,
						Content = "198.51.100.4",
		...
		public async Task ShouldListDnsRecords()
		{
			...
			_clientMock.Verify(m => m.GetAsync<IReadOnlyList<DnsRecord>>($"zones/{ZoneId}/dns_records", null, It.IsAny<CancellationToken>()), Times.Once);
		...
		public async Task ShouldListDnsRecordsWithFilter()
		{
			...
			_clientMock.Verify(m => m.GetAsync<IReadOnlyList<DnsRecord>>($"zones/{ZoneId}/dns_records", filter, It.IsAny<CancellationToken>()), Times.Once);
			_clientMock.VerifyNoOtherCalls();
		}
		...
		private ICloudflareClient GetClient()
		{
			...
			_clientMock
				.Setup(m => m.GetAsync<IReadOnlyList<DnsRecord>>(It.IsAny<string>(), It.IsAny<IQueryParameterFilter>(), It.IsAny<CancellationToken>()))
